...
int pos[MAXN], ans[MAXN], first[MAXN], last;

void update(int l, int r, int qid) { // use to scan and calculate and update the ans[qid]
    for (int i = r; i >= l; --i) {
        pos[arr[i]] = max(pos[arr[i]], i);
        ans[qid] = max(ans[qid], pos[arr[i]] - i);
    }
    for (int i = r; i >= l; --i) {
        if (pos[arr[i]] == i) pos[arr[i]] = 0;
    }
}
    ...
    while (p <= m) {
        ...
        while (i <= m && belong[qs[i].l] == block) {
            if (qs[i].r <= br[block]) { // at the same block so we'd better calculate directly
                ...
                while (r < qs[i].r) {
                    ++r;
                    if (!first[arr[r]]) first[arr[r]] = r;
                    pos[arr[r]] = r;
                    last = max(last, pos[arr[r]] - first[arr[r]]);
                }
                ans[qs[i].id] = last;
