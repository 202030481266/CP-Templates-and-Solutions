...
void solve() {
	...
	vec<vec<double>> dp(1 << n, vec<double>(x + 1));
	// dp[mask][i] = dp[mask][i - c[j]] * (100 - p[j]) * 0.01 + (dp[mask ^ 1 << j][i - c[j]] + s[j]) * p[j] * 0.01;
	for (int S = 1; S < 1 << n; ++S) {
		for (int j = 1; j <= x; ++j) {
			for (int i = 0; i < n; ++i) {
				if (S >> i & 1 && j >= c[i]) {
					dp[S][j] = max(dp[S][j], dp[S][j - c[i]] * (100 - p[i]) * 0.01 + (dp[S ^ 1 << i][j - c[i]] + s[i]) * p[i] * 0.01);
				}
			}
		}
	}

	cout << fixed << setprecision(6) << dp[(1 << n) - 1][x] << '\n';
}
