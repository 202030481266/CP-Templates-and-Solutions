...
void solve() {
    ...
    for (int& v : arr) cin >> v;
    ranges::sort(arr);
    arr.erase(ranges::unique(arr).begin(), arr.end());
    // if 1 b c alice win, then a b c (a > 1) must bob win, same with other situations
    n = arr.size();
    int one = 1;
    for (int i = n - 2; i >= 0; --i) {
        if (arr[i + 1] == arr[i] + 1) one ^= 1;
        else one = 0;
    }
    cout << (arr[0] > 1 || one ? "Alice" : "Bob") << '\n';
}
