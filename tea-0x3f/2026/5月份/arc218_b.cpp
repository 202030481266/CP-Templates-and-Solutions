// Problem: B - All Minus
// Contest: AtCoder - AtCoder Regular Contest 218
// URL: https://atcoder.jp/contests/arc218/tasks/arc218_b
...
void solve() {
    ...
    for (int i = n - 1; i >= 0; --i) {
        ...
        if (a[i].first > 1) cur = 1;
        else {
            if (a[i].first == 1) {
                if (a[i].second > 1) cur = 0;
            }
            else {
                if (a[i].second > 1) cur = 1;
                else cur ^= 1;
            }
        }
    }
