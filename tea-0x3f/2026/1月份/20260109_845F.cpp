...
void solve() {
	...
	for (int i = 0; i < n; ++i) {
		for (int j = 0; j < m; ++j) g[i][j] = readchar();
	}
	if (n < m) {
		swap(n, m);
		vector trans(n, vector<char>(m));
		for (int i = 0; i < n; ++i) {
			for (int j = 0; j < m; ++j) trans[i][j] = g[j][i];
		}
		g = move(trans);
	}
	vector dp(2, vector(2, vector(2, vector<int>(1 << m))));
	int pre = 0, cur = 1;
	dp[1][0][0][0] = 1;
	for (int i = 0; i < n; ++i) {
		// should not have left guard
		for (int x = 0; x < 2; ++x) {
			for (int k = 0; k < 1 << m; ++k) {
				dp[cur][x][0][k] = (dp[cur][x][0][k] + dp[cur][x][1][k]) % MOD;
				dp[cur][x][1][k] = 0;
			}
		}
		for (int j = 0; j < m; ++j) {
			...
			for (int x = 0; x < 2; ++x) {
				for (int y = 0; y < 2; ++y) {
					for (int k = 0; k < 1 << m; ++k) dp[cur][x][y][k] = 0;
				}
			}
			for (int s = 0; s < 1 << m; ++s) {
				if (g[i][j] == 'x') {
					int nxt = s & ~(1 << j);
					dp[cur][0][0][nxt] = (dp[cur][0][0][nxt] + (dp[pre][0][1][s] + dp[pre][0][0][s]) % MOD) % MOD;
					dp[cur][1][0][nxt] = (dp[cur][1][0][nxt] + (dp[pre][1][1][s] + dp[pre][1][0][s]) % MOD) % MOD;
				}
				else {
					// not take, won't influence the empty space
					// 1. have left guard
					dp[cur][0][1][s] = (dp[cur][0][1][s] + dp[pre][0][1][s]) % MOD;
					dp[cur][1][1][s] = (dp[cur][1][1][s] + dp[pre][1][1][s]) % MOD;
					// 2. no left guard
					if (s >> j & 1) {
						dp[cur][0][0][s] = (dp[cur][0][0][s] + dp[pre][0][0][s]) % MOD;
						dp[cur][1][0][s] = (dp[cur][1][0][s] + dp[pre][1][0][s]) % MOD;
					}
					// not take and be the empty space
					if (!(s >> j & 1)) {
						dp[cur][1][0][s] = (dp[cur][1][0][s] + dp[pre][0][0][s]) % MOD;
					}
					// place a guard
					int nxt = s | 1 << j;
					dp[cur][0][1][nxt] = (dp[cur][0][1][nxt] + (dp[pre][0][0][s] + dp[pre][0][1][s]) % MOD) % MOD;
					dp[cur][1][1][nxt] = (dp[cur][1][1][nxt] + (dp[pre][1][0][s] + dp[pre][1][1][s]) % MOD) % MOD;
				}
			}
		}
	}
	...
	for (int i = 0; i < 2; ++i) {
		for (int j = 0; j < 2; ++j) {
			for (int k = 0; k < 1 << m; ++k) ans = (ans + dp[cur][i][j][k]) % MOD;
		}
	}
	writeln(ans);
}
