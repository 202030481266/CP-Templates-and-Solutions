...
void solve() {
    ...
    for (int i = 0; i < 2; ++i) {
        for (int j = 0; j < n; ++j) {
            if (j == 0) mn[i][j] = mx[i][j] = arr[i][j];
            else {
                mn[i][j] = min(mn[i][j - 1], arr[i][j]);
                mx[i][j] = max(mx[i][j - 1], arr[i][j]);
            }
        }
        for (int j = n - 1; j >= 0; --j) {
            if (j == n - 1) sn[i][j] = sx[i][j] = arr[i][j];
            else {
                sn[i][j] = min(sn[i][j + 1], arr[i][j]);
                sx[i][j] = max(sx[i][j + 1], arr[i][j]);
            }
        }
    }
    vi right(2 * n + 1, INF);
    for (int i = 0; i < n; ++i) {
        int mn_val = min(mn[0][i], sn[1][i]);
        int mx_val = max(mx[0][i], sx[1][i]);
        right[mn_val] = min(right[mn_val], mx_val);
    }
    vp st(n + 1);
    int ptr = 0;
    for (int i = 1; i <= 2 * n; ++i) {
        if (right[i] == INF) continue;
        while (ptr > 0 && st[ptr - 1].second >= right[i]) --ptr;
        st[ptr++] = {i, right[i]};
    }
    ll ans = 0;
    int last = 0;
    for (int i = 0; i < ptr; ++i) {
        ans += 1ll * (st[i].first - last) * (2 * n - st[i].second + 1);
        last = st[i].first;
    }
    cout << ans << '\n';
}
