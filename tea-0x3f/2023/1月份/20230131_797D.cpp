...
void solve() {
	...
	for (int i = 1; i <= n; ++i) {
		...
		read(son[i][0]);
		read(son[i][1]);
		if (son[i][0] != -1) fa[son[i][0]] = i;
		if (son[i][1] != -1) fa[son[i][1]] = i;
	...
	auto dfs = [&](auto&& self, int u, int l, int r) -> void {
		if (u == -1) return;
		if (l < r - 1 && a[u] > l && a[u] < r) num[rk[u]] = 1;
		self(self, son[u][0], l, min(a[u], r));
		self(self, son[u][1], max(l, a[u]), r);
	};
	int root = 0;
	for (int i = 1; i <= n; ++i) {
		if (fa[i] == 0) {
