...
void solve() {
	...
	for (int i = 1; i <= n; ++i) {
		if (s[i] == '*' || s[i] == 'P') arr[tot++] = i;
	}
	int p = 0, last = 0, st;
	n = tot;
	while (p < n && s[arr[p]] != 'P') ++p;
	st = p;
	while (p < n) {
		int i = p + 1;
		while (i < n && s[arr[i]] != 'P') ++i;
		if (p == st) {
			if (p > 0) {
				for (int j = p; j < i; ++j) {
					dp[j] = arr[j] - arr[0] + min(arr[j] - arr[p], arr[p] - arr[0]);
				}
			}
			else {
				for (int j = p; j < i; ++j) {
			...
			for (int j = p; j < i; ++j) {
				while (last + 1 < p && dp[last] < arr[j] - arr[last + 1] + min(arr[j] - arr[p], arr[p] - arr[last + 1])) {
					dp[j] = min(dp[j], arr[j] - arr[last + 1] + min(arr[j] - arr[p], arr[p] - arr[last + 1]));
					++last;
				}
				dp[j] = min(dp[j], max(dp[last], arr[j] - arr[last + 1] + min(arr[j] - arr[p], arr[p] - arr[last + 1])));
			}
		}
		last = p;
		p = i;
