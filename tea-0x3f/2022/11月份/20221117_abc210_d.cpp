...
void solve() {
	...
	vll mn1(n + 1, vl(m + 1, LLINF));
	vll mn2(n + 1, vl(m + 1, LLINF));
	ll ans = LLINF;
	for (int i = n; i >= 1; --i) {
		for (int j = m; j >= 1; --j) {
			if (i + 1 <= n) mn1[i][j] = min(mn1[i][j], mn1[i + 1][j]);
			if (j + 1 <= m) mn1[i][j] = min(mn1[i][j], mn1[i][j + 1]);
			if (mn1[i][j] < LLINF) ans = min(ans, arr[i][j] - c * i - c * j + mn1[i][j]);
			mn1[i][j] = min(mn1[i][j], arr[i][j] + c * i + c * j);
		}
	}
	for (int i = n; i >= 1; --i) {
		for (int j = 1; j <= m; ++j) {
			if (j - 1 >= 1) mn2[i][j] = min(mn2[i][j], mn2[i][j - 1]);
			if (i + 1 <= n) mn2[i][j] = min(mn2[i][j], mn2[i + 1][j]);
			if (mn2[i][j] < LLINF) ans = min(ans, arr[i][j] - c * i + c * j + mn2[i][j]);
			mn2[i][j] = min(mn2[i][j], arr[i][j] + c * i - c * j);
		}
	}
	writeln(ans);
}
