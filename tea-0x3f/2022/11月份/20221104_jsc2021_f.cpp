...
void solve() {
	...
	vector<fenwick_tree<int>> sz(2, fenwick_tree<int>(cur + 1));
	vector<fenwick_tree<ll>> tree(2, fenwick_tree<ll>(cur + 1));
	vii arr(2);
	arr[0].resize(n + 1);
	arr[1].resize(m + 1);
	sz[0].add(0, n);
	sz[1].add(0, m);
	ll ans = 0;
	auto cal = [&](int k, int val) -> ll {
		int ge_sz = sz[k ^ 1].sum(0, val);
		ll res = 1ll * num[val] * ge_sz + tree[k ^ 1].sum(0, cur + 1) - tree[k ^ 1].sum(0, val); // [val, cur]
		return res;
	};
	for (int i = 0; i < q; ++i) {
		int k = qs[i][0] - 1, pre = arr[k][qs[i][1]];
		ll new_val = cal(k, rk[i]), old_val = cal(k, pre);
		ans += new_val - old_val;
		// update the array
		sz[k].add(pre, -1);
		tree[k].add(pre, -num[pre]);
		arr[k][qs[i][1]] = rk[i];
		sz[k].add(rk[i], 1);
		tree[k].add(rk[i], num[rk[i]]);
		writeln(ans);
	}
}
