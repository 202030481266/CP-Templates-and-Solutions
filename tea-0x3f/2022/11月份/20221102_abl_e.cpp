...
S op(S lson, S rson) {
	return { (lson.val * pw[rson.sz] % MOD + rson.val) % MOD, lson.sz + rson.sz };
}
...
S mapping(F f, S s) {
	if (f == -1) return s;
	return { sum[s.sz - 1] * f % MOD, s.sz };
}

F composition(F new_op, F old_op) {
	if (new_op == -1) return old_op;
	return new_op;
}
...
void solve() {
	...
	for (int i = 0, L, R, D; i < m; ++i) {
		...
		read(D);
		seg.apply(L, R + 1, D);
		FastIO::writeln(seg.all_prod().val);
	}
}
