...
void solve() {
	...
	for (auto& v : vec) read(v.first), read(v.second);
	for (auto& [x, y] : vec) {
		if (x == 0 && y == 0) continue;
		if (x < 0) x *= -1, y *= -1;
		ll g = gcd(x, y);
	...
	vector<pii> group;
	int N = n, ax = 0, ay = 0, zero = 0;
	for (auto v : vec) {
		if (v.first != 0 && v.second != 0) {
			if (v.second > 0) up.emplace_back(v);
			else down.emplace_back(v);
		}
		else {
			if (v.first == 0 && v.second == 0) --N, ++zero; //(0,0)
			else if (v.first != 0) ++ax;
			else ++ay;
		}
	}
	if (ax > 0 && ay > 0) group.emplace_back(ax, ay);
	ranges::sort(up);
	ranges::sort(down);
	int p = 0;
	while (p < up.size()) {
		int i = p + 1;
		while (i < up.size() && up[i] == up[p]) ++i;
		auto pr = make_pair(up[p].second, -up[p].first);
		auto it = lower_bound(down.begin(), down.end(), pr);
		if (it != down.end() && *it == pr) {
			int pos = it - down.begin(), j = pos + 1;
			while (j < down.size() && down[j] == pr) ++j;
			group.emplace_back(i - p, j - pos);
		}
		p = i;
	}
	for (auto& [g1, g2] : group) {
		N -= g1 + g2;
	}
	vector<mint> pw(n + 1);
	pw[0] = 1;
	...
	mint ans = pw[N];
	for (auto& [g1, g2] : group) {
		mint res = pw[g1] + pw[g2] - 2;
		ans = ans + ans * res;
	} 
	ans += zero - 1; // non empty
	writeln(ans.val());
}
