...
void solve() {
	...
	for (int i = 0, u, v; i < q; ++i) {
		read(u);
		read(v);
		qs[u - 1].emplace_back(v - 1, i);
	}
	vector<pii> intervals;
	unordered_set<int> s, sa, sb;
	int last = 0;
	s.insert(b[0]);
	for (int i = 1; i < n; ++i) {
		if (!s.contains(b[i])) {
			intervals.emplace_back(last, i);
			s.insert(b[i]);
			last = i;
		}
	}
	...
	for (int i = 0, j = -1; i < n; ++i) {
		if (!sa.contains(a[i])) {
			sa.insert(a[i]);
			if (sb.contains(a[i])) --y;
			if (!sb.contains(a[i])) ++x;
		}
		while (j + 1 < intervals.size() && x > 0) {
			++j;
			int r = intervals[j].first;
			if (!sb.contains(b[r])) {
				sb.insert(b[r]);
				if (sa.contains(b[r])) --x;
				if (!sa.contains(b[r])) ++y;
			}
		}
		if (x == 0 && y == 0) {
			for (auto [pos, qid] : qs[i]) {
				if (pos >= intervals[j].first && pos < intervals[j].second) ans[qid] = 1;
			}
		}
	}
	for (int i = 0; i < q; ++i) {
		if (ans[i]) writeln("Yes");
		else writeln("No");
	}
}
