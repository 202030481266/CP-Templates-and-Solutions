...
void solve() {
	...
	read(m);
	int pre = 0;;
	for (int i = 1; i <= m; ++i) dp[pre][i][0][0] = 1;
	for (int t = 1; t < n; ++t) {
		memset(dp[pre ^ 1], 0, sizeof(dp[pre ^ 1]));
		for (int i = 1; i <= m; ++i) {
			for (int j = i; j <= m; ++j) {
				dp[pre ^ 1][i][0][0] = (dp[pre ^ 1][i][0][0] + dp[pre][j][0][0]) % MOD;
			}
		}
		for (int i = 1; i <= m; ++i) {
			for (int k = i + 1; k <= m; ++k) {
				dp[pre ^ 1][i][k][0] = (dp[pre ^ 1][i][k][0] + dp[pre][i][0][0]) % MOD;
			}
		}
		for (int i = 1; i <= m; ++i) {
			for (int j = i + 1; j <= m; ++j) {
				...
				for (int k = 1; k <= i; ++k) {
					dp[pre ^ 1][k][j][0] = (dp[pre ^ 1][k][j][0] + dp[pre][i][j][0]) % MOD;
				}
				for (int k = i + 1; k <= j; ++k) {
					dp[pre ^ 1][i][k][0] = (dp[pre ^ 1][i][k][0] + dp[pre][i][j][0]) % MOD;
				}
			}
		}
		for (int i = 1; i <= m; ++i) {
			for (int j = i + 1; j <= m; ++j) {
				if (!dp[pre][i][j][0]) continue;
				for (int k = j + 1; k <= m; ++k) {
					dp[pre ^ 1][i][j][k] = (dp[pre ^ 1][i][j][k] + dp[pre][i][j][0]) % MOD;
				}
			}
		}
		for (int i = 1; i <= m; ++i) {
			for (int j = i + 1; j <= m; ++j) {
				for (int k = j + 1; k <= m; ++k) {
					if (!dp[pre][i][j][k]) continue;
					// v <= i, i < v <= j, j < v <= k
					for (int v = 1; v <= i; ++v) {
						dp[pre ^ 1][v][j][k] = (dp[pre ^ 1][v][j][k] + dp[pre][i][j][k]) % MOD;
					}
					for (int v = i + 1; v <= j; ++v) {
						dp[pre ^ 1][i][v][k] = (dp[pre ^ 1][i][v][k] + dp[pre][i][j][k]) % MOD;
					}
					for (int v = j + 1; v <= k; ++v) {
						dp[pre ^ 1][i][j][v] = (dp[pre ^ 1][i][j][v] + dp[pre][i][j][k]) % MOD;
					}
				}
			}
		}
	...
	for (int i = 1; i <= m; ++i) {
		for (int j = i + 1; j <= m; ++j) {
			for (int k = j + 1; k <= m; ++k) ans = (ans + dp[pre][i][j][k]) % MOD;
		}
	}
	writeln(ans);
}
