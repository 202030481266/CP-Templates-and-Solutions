...
double findMedian(std::vector<int>& v) {
	...
	if (n % 2 != 0) {
		...
		auto mid2 = v.begin() + n / 2;     // 上中位数位置
		auto mid1 = mid2 - 1;              // 下中位数位置
		std::nth_element(v.begin(), mid2, v.end());
		int val2 = *mid2;
		int val1 = *std::max_element(v.begin(), mid2);
...
void solve() {
	...
	double m1 = findMedian(a), m2 = findMedian(b);
	if (n & 1) writeln(static_cast<int>(m2 - m1 + 1));
	else writeln(static_cast<int>((m2 - m1) * 2 + 1));
}
