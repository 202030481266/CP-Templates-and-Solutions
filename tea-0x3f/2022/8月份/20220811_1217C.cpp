...
void solve() {
	string s;
	read(s);
	int n = s.size(), zero = 0, m = static_cast<int>(log2(n)) + 1;
	ll ans = 0;
	uint cur = 0;
	for (int i = 0; i < n; ++i) {
		cur = cur << 1 | (s[i] - '0');
		...
		int w = bit_width(cur), pre = min(32, i + 1) - w;
		uint last = 0;
		for (int j = 0; j < min(w, m); ++j) {
			uint tmp = (1u << j + 1) - 1 & cur;
			if (tmp == last) continue;
			last = tmp;
			int need = tmp - bit_width(tmp), tot;
			if (!tmp || need < 0) continue;
			if (cur >> j + 1 == 0) tot = pre + zero;
			else tot = bit_width(lowbit(cur >> j + 1)) - 1;
			if (tot >= need && need >= j + 1 - bit_width(tmp)) ++ans;
		}
	}
	writeln(ans);
}
