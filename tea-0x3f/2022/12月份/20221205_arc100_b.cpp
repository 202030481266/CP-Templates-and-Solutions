// Problem: D - Equal Cut
// Contest: AtCoder - AtCoder Beginner Contest 102
// URL: https://atcoder.jp/contests/abc102/tasks/arc100_b
...
void solve() {
	...
	for (int i = 1; i < n; ++i) {
		...
		if (cur * 2 - sum < sum - cur * 2 + arr[l - 1] * 2) {
			mx[i] = cur;
			mn[i] = sum - cur;
		}
		else {
			mx[i] = sum - cur + arr[l - 1];
			mn[i] = cur - arr[l - 1];
		}
	}
	l = n - 1;
	...
	for (int i = n - 2; i >= 2; --i) {
		...
		ans = min(ans, max(mx[i - 1], x) - min(y, mn[i - 1]));
	}
	writeln(ans);
}
