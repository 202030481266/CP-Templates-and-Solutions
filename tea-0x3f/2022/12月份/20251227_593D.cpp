...
int rt[MAXN], eid[MAXN], nid[MAXN];
ll val[MAXN];
int head[MAXN], to[MAXN << 1], nxt[MAXN << 1], weight[MAXN << 1], tot = 0;
...
void solve() {
	...
	while (m--) {
		...
		if (op == 1) {
			...
			int lcaf = lca(x, y);
			ll cur = 1, node = find(x), ans = -1;
			while (dep[node] > dep[lcaf]) {
				if (val[eid[node]] > value / cur) {
					ans = 0;
					break;
				}
				cur *= val[eid[node]];
			...
			read(e);
			read(new_val);
			if (val[e] > 1 && new_val == 1) {
				int node = nid[e];
				rt[node] = rt[fa[node][0]];
			}
			val[e] = new_val;
		}
	}
