...
void solve() {
	...
	vi ans(n + 1);
	int p = 1;
	for (int i = b, j = 0; j < x; ++j, i += b) {
		for (int k = i; k > i - b; --k, ++p) ans[p] = k;
	}
	int rem = n - x * b, rg = a - x;
	if (rg > 1) {
		for (int i = n; i >= n - rg + 2; --i) ans[i] = i;
	}
	for (int i = n - rg + 1; i > x * b; --i, ++p) ans[p] = i;
