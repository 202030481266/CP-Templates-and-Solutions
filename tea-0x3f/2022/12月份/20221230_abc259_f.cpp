...
void solve() {
	...
	auto dfs = [&](auto&& self, int u, int fa) -> void {
		...
		for (auto [v, w] : g[u]) {
			if (v == fa) continue;
			if (dp[v][1] + w > dp[v][0]) a.emplace_back(dp[v][1] + w - dp[v][0], v);
			else {
				dp[u][0] += dp[v][0];
				dp[u][1] += dp[v][0];
			}
		...
		int m = a.size();
		for (int i = 0; i < m; ++i) {
			if (i < d[u]) dp[u][0] += dp[a[i].second][0] + a[i].first;
			else dp[u][0] += dp[a[i].second][0];
			if (i < d[u] - 1) dp[u][1] += dp[a[i].second][0] + a[i].first;
			else dp[u][1] += dp[a[i].second][0];
		}
	};
	ll ans = 0;
	for (int i = 1; i <= n; ++i) {
