...
void solve() {
	...
	for (int i = 0; i < n; ++i) {
		read(ops[i].first);
		if (ops[i].first == "DELETE") continue; // delete operation doesn't have val
		read(ops[i].second);
		arr.emplace_back(ops[i].second, i);
	}
	ranges::sort(arr);
	int rank = -1;
	for (int i = 0; i < (int)arr.size(); ++i) {
		if (i == 0 || arr[i].first != arr[i - 1].first) ++rank;
		rk[arr[i].second] = rank;
	}
	vii g(n);
	vii pos(rank + 1);
	vi ans(n);
	vi cur;
	for (int i = n - 1; i >= 0; --i) {
		if (ops[i].first == "LOAD") pos[rk[i]].push_back(i);
		if (ops[i].first == "SAVE") {
			g[i] = pos[rk[i]];
			pos[rk[i]].clear();
		}
	...
	int pre = -1, f = 0;
	for (int i = 0; i < n; ++i) {
		if (ops[i].first == "SAVE") vis[rk[i]] = 1;
		if (ops[i].first == "LOAD") {
			if (vis[rk[i]]) f = ops[i].second;
			else f = 0;
		}
		if (f == 0) {
			if (pre == -1) pre = i;
	...
	auto dfs = [&](auto&& self, int u, int f) -> void {
		if (u == n) return;
		string ins = ops[u].first;
		if (ins == "ADD") {
			int val = ops[u].second;
			cur.push_back(val);
			ans[u] = val;
			if (f) {
				self(self, u + 1, f);
				cur.pop_back(); // f == 0 is useless
			}
			else self(self, nxt[u], f);
		}
		if (ins == "DELETE") {
			int tmp = cur.empty() ? -1 : cur.back();
			if (tmp != -1) cur.pop_back();
			ans[u] = cur.empty() ? -1 : cur.back();
			if (f) {
				self(self, u + 1, f);
				if (tmp != -1) cur.push_back(tmp); // f == 0 is useless
			}
			else self(self, nxt[u], f);
		}
		if (ins == "LOAD") {
		...
		if (ins == "SAVE") {
			ans[u] = cur.empty() ? -1 : cur.back();
			for (int v : g[u]) self(self, v, ops[u].second);
			if (f) self(self, u + 1, f);
			else self(self, nxt[u], f);
		}
	};
	dfs(dfs, 0, 0);
