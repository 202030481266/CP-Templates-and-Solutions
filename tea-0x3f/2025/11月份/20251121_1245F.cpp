...
ll dfs(int p, bool limitLow, bool limitHigh, bool is_same) {
	if (p < 0) return !is_same;
	if (dp[p][limitLow][limitHigh][is_same] != -1) return dp[p][limitLow][limitHigh][is_same];
	ll& res = dp[p][limitLow][limitHigh][is_same];
	res = 0;
	int lo = 0, up = 1;
	if (limitHigh) up = R >> p & 1;
	if (limitLow) lo = L >> p & 1;
	for (int i = 0; i <= up; ++i) {
		for (int j = lo; j <= 1; ++j) {
			if (i == j && i) continue;
			if (is_same && j > i) continue;
			res += dfs(p - 1, limitLow && j == lo, limitHigh && i == up, is_same && i == j);
		}
	}
	return res;
}
