...
int lp[MAXN], cnt[MAXN], tot[MAXN];
int n, m, arr[MAXN], num[MAXN];
...
void solve() {
	...
	for (int i = 1; i <= n; ++i) {
		...
		if (num[i] > 1 && tot[num[i]] && tot[num[i]] % 2 == 0) {
			sum += tot[num[i]];
			tot[num[i]] = 0;
		}
	}
	...
	while (m--) {
		ll w;
		read(w);
		if (w > 0) cout << max(ans, sum) << '\n';
		else cout << ans << '\n';
	}
	for (int i = 1; i <= n; ++i) tot[num[i]] = 0;
}
