...
void solve() {
	...
	for (char ch : s) {
		int c = ch - 'a';
		for (int i = 0; i <= m; ++i) {
			for (int j = 0; j <= m; ++j) dp[cur][i][j] = dp[pre][i][j];
		}
		for (int p : g[c]) {
			// match pre part
			if (dp[pre][p - 1][0] != -1) { // suf part is empty string
				dp[cur][p][0] = 0;
			}
			for (int j = p + 1; j <= m; ++j) { // suf part
				if (dp[pre][p - 1][j] != -1) 
					dp[cur][p][j] = max(dp[cur][p][j], dp[pre][p - 1][j]);
			}
		}
		for (int p : g[c]) {
			// match suf part
			for (int i = 0; i < p; ++i) {
				if (dp[pre][i][0] != -1) dp[cur][i][p] = max(dp[cur][i][p], 1);
				if (dp[pre][i][p - 1] != -1) {
					dp[cur][i][p] = max(dp[cur][i][p], dp[pre][i][p - 1] + 1);
				}
			}
		}
		swap(pre, cur);
	...
	for (int i = 0; i <= m; ++i) {
		if (dp[pre][i][m] != -1 && dp[pre][i][m] == m - i) {
			cout << "YES\n";
			return;
		}
	}
