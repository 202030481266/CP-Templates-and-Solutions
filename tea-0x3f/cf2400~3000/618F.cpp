...
// 对于 [1,i] 的值域来说，如果 sa + sb >= i + 1 有解，那么可以使用数学归纳法证明:
// 对于 [1, i + 1]来说，sa + sb >= i + 2，如果两边的 i + 1 位置上的值都有 1， 那么当前 sa + sb >= i，将这两个数字做差
// 因为 1 <= a[i + 1] <= i + 1, 1 <= b[i + 1] <= i + 1, abs(a[i + 1] - b[i + 1]) <= i，也就是说一定会落到 [1, i]的某个位置上
// 所以可得 sa + sb 在 [1, i] 上依然有 sa + sb >= i + 1，即有解
