...
void solve() {
	...
	ranges::sort(ids, [&](int i, int j) { return a[i] == a[j] ? b[i] < b[j] : a[i] < a[j]; });
	vi ans;
	vi use(n);
	for (int i = 0; i + 1 < n; i += 2) {
		if (b[ids[i]] > b[ids[i + 1]]) {
			...
			ans.push_back(ids[i + 1]);
			use[i + 1] = 1;
		}
	}
	if (n & 1) {
		ans.push_back(ids[n - 1]);
	}
	else {
		if (!use[n - 1]) ans.push_back(ids[n - 1]);
		else ans.push_back(ids[n - 2]);
	}
	cout << ans.size() << '\n';
