...
void solve() {
	...
	read(b);
	int n = s.size();
	for (int i = 0; i < 10; ++i) pos[i] = n;
	for (int i = n - 1; i >= 0; --i) {
		for (int j = 0; j < 10; ++j) nxt[i][j] = pos[j];
		pos[s[i] - '0'] = i;
	}
	int ptr = -1;
	for (int i = 0; i < m && ptr < n; ++i) {
		int p = 0;
		if (ptr < 0) {
			for (int j = a[i]; j <= b[i]; ++j) p = max(p, pos[j - '0']);
		}
		else {
			for (int j = a[i]; j <= b[i]; ++j) p = max(p, nxt[ptr][j - '0']);
		}
		ptr = p;
	}
	if (ptr < n) cout << "NO\n";
