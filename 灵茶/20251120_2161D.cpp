...
void solve() {
	...
	for (int i = 1; i <= m; ++i) {
		if (i == 1 || arr[i] - arr[i - 1] > 1) {
			if (i > 1) ans += min(dp[1], dp[g[arr[i - 1]].size() + 1]);
			for (int j = 1; j <= g[arr[i]].size() + 1; ++j) dp[j] = j - 1;
		}
		else {
			int pre = g[arr[i - 1]].size(), cur = g[arr[i]].size();
			f[cur + 1] = cur + min(dp[1], dp[g[arr[i - 1]].size() + 1]);
			for (int k = cur, j = pre, mn = dp[pre + 1]; k >= 1; --k) {
				while (j >= 1 && g[arr[i - 1]][j - 1] > g[arr[i]][k - 1]) mn = min(mn, dp[j--]);
				if (k < cur) f[k] = min(f[k + 1] - 1, mn + cur - 1);
				else f[k] = mn + cur - 1;
			}
			for (int i = 1; i <= cur + 1; ++i) dp[i] = f[i];
		}
	}
	if (m > 1) ans += min(dp[1], dp[g[arr[m]].size() + 1]);
	writeln(ans);
} 
