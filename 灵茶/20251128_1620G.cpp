...
int n;
int arr[24][26], cnt[26];
ll common[1 << 23], tot[1 << 23];
int sum[1 << 23];
	...
	for (int i = 0; i < 1 << n; ++i) {
		...
		for (int j = 0; j < n; ++j) {
			if (i >> j & 1) {
				sum[i] += j + 1;
				for (int k = 0; k < 26; ++k) {
					if (cnt[k] > arr[j][k]) cnt[k] = arr[j][k];
				}
			}
		}
		ll tmp = 1;
		for (int j = 0; j < 26; ++j) {
			if (cnt[j] < INF) tmp = tmp * (cnt[j] + 1) % MOD;
		}
		common[i] = tmp; // 包含了空的集合
	}
	// SOS DP
	for (int i = 1; i < 1 << n; ++i) {
		int one = __builtin_popcount(i);
		tot[i] = one & 1 ? common[i] : -common[i];
	}
	for (int i = 0; i < n; ++i) {
		for (int status = 1; status < 1 << n; ++status) {
			if (status >> i & 1) tot[status] = (tot[status] + tot[status ^ 1 << i]) % MOD;
		}
	}
	ll ans = 0;
	tot[0] = 1;
	for (int i = 0; i < 1 << n; ++i) {
		tot[i] = (tot[i] % MOD + MOD) % MOD;
		ans ^= tot[i] * __builtin_popcount(i) * sum[i];
	}
	writeln(ans);
}
