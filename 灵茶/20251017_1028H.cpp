...
// 虽然是2900的题目，但是难点只有pre数组的使用
static constexpr int MAXV = 5032108;
int lp[MAXV], cnt[MAXV];
int n, q, arr[MAXN], num[MAXN];
int pre[MAXN][15], pos[MAXV][8];
int factor[10], fLen = 0;
int oddfactor[10], oddLen = 0;
int mn[15][MAXN][20];
...
void init() {
	...
	for (int i = 2; i < MAXV; ++i) {
		if (!lp[i]) {
			lp[i] = i;
			if (1ll * i * i < MAXV) {
				for (int j = i * i; j < MAXV; j += i) lp[j] = i;
			}
		}
	}
}
...
void solve() {
	...
	for (int i = 1, x = 0, y = 1; i <= n; ++i) {
		...
		for (int j = 0; j < fLen; ++j) {
			if (cnt[factor[j]] & 1) oddfactor[oddLen++] = factor[j];
			cnt[factor[j]] = 0;
		}
		for (int j = 0; j < (1 << oddLen); ++j) {
			int cost = oddLen - __builtin_popcount(j);
			for (int k = 0; k < oddLen; ++k) {
				if (j >> k & 1) y *= oddfactor[k];
			}
			for (int k = cost; k < 15; ++k) pre[i][k] = max(pre[i][k], pos[y][k - cost]);
			pos[y][cost] = i;
			y = 1;
		}
		fLen = oddLen = 0;
	...
	int t = log(n) / log(2) + 1;
	for (int x = 0; x < 15; ++x) {
		for (int i = 1; i <= n; ++i) mn[x][i][0] = pre[i][x];
	}
	for (int x = 0; x < 15; ++x) {
		for (int i = 1; i < t; ++i) {
			for (int j = 1; j + (1 << i) - 1 <= n; ++j) mn[x][j][i] = max(mn[x][j][i - 1], mn[x][j + (1 << i - 1)][i - 1]);
		}
	}
	int ql = 0, qr = 0, k = 0;
	while (q--) {
		...
		int flag = 1;
		for (int i = 0; i < 15; ++i) {
			if (max(mn[i][ql][k], mn[i][qr - (1 << k) + 1][k]) >= ql) {
				writeln(i);
				flag = 0;
				break;
			}
