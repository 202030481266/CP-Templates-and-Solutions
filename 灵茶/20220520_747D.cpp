...
void solve() {
	...
	while (p <= n) {
		int i = p + 1;
		while (i <= n && (arr[i] ^ arr[p]) == 0) ++i;
		if (arr[p] ^ cur) ++ans; // need to change
		if (cur && arr[p] == 0 && i <= n) cost.push_back(i - p); // hope for continue use
		cur = arr[p];
		p = i;
	}
	p = n;
	while (p >= 1 && arr[p] == 0) --p;
	int rem = k - sum, res = (p < n && p >= 1 && rem >= n - p ? ans - 1 : inf);
	int r = (res == inf ? -inf : rem - n + p);
