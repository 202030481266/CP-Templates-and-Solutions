...
int n, a[maxn], b[maxn], c[maxn], lo[maxn];
ll sum;
...
bool check(int val) {
	...
	if (x > z + y || y > z + x) return false;
	int tmp = (z - abs(x - y)) / 2;
	if (y >= x) {
		// y - x 个左边，然后是 z - y + x 个进行安排，(z - y + x) / 2放到左边，然后其他都是 val
		for (int i = 1; i <= y - x + tmp; ++i) use += lo[i];
		use += 1ll * (z - y + x - tmp) * val;
	}
	else {
		// x - y 个 val, 然欧 z - y + x 个安排，(z - y + x) / 2 放到左边
		for (int i = 1; i <= tmp; ++i) use += lo[i];
		use += 1ll * (z - tmp) * val;
	}
	return use <= sum;
}
...
void solve() {
	...
	r = c[(n + 1) / 2] + 1;
	iota(c + 1, c + 1 + n, 1);
	sort(c + 1, c + 1 + n, [&](int i, int j) { return a[i] < a[j]; });
	while (l + 1 < r) {
		int mid = (l + r) >> 1;
		if (check(mid)) l = mid;
		else r = mid;
