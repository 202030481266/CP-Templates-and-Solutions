...
void solve() {
	...
	if (y == 0) y = 1;
	int dx = 0, dy = 0;
	int row = (arr[1] - 1) / y + 1, col = (arr[1] - 1) % y + 1;
	x = row;
	for (int i = 2; i <= n; ++i) {
		if (abs(arr[i] - arr[i - 1]) == y) dx += (arr[i] < arr[i - 1] ? -1 : 1);
		else dy += (arr[i] < arr[i - 1] ? -1 : 1);
		if (col + dy < 1 || row + dx < 1 || col + dy > y) {
			writeln("NO");
			return;
		}
		x = max(x, row + dx);
	}
	if (x <= 1000'000'000 && y <= 1000'000'000) {
		writeln("YES");
		write(x, ' ');
		writeln(y);
	}
