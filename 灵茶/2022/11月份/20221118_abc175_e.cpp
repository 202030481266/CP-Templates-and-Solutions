...
void solve() {
	...
	vector<vll> dp(n + 1, vll(m + 1, vl(4)));
	for (int i = 1; i <= n; ++i) {
		for (int j = 1; j <= m; ++j) {
			ll mx = 0;
			for (int k = 0; k <= 3; ++k) mx = max(mx, dp[i - 1][j][k]);
			dp[i][j][0] = max(dp[i][j - 1][0], mx);
			dp[i][j][1] = max(dp[i][j - 1][1], max(mx + arr[i][j], dp[i][j - 1][0] + arr[i][j]));
			dp[i][j][2] = max(dp[i][j - 1][2], dp[i][j - 1][1] + arr[i][j]);
			dp[i][j][3] = max(dp[i][j - 1][3], dp[i][j - 1][2] + arr[i][j]);
		}
	}
	ll ans = 0;
