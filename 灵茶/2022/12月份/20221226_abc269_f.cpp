...
ll cal(ll row, ll cl, ll cr) {
	ll len = cr - cl + 1;
	if (row & 1) { 
		ll L = (cl / 2) * (cl / 2);
		ll R = ((cr + 1) / 2) * ((cr + 1) / 2);
		ll cnt = len % 2 == 0 || cl % 2 == 0 ? len / 2 : len / 2 + 1;
		ll base = R - L;
		return (base % MOD + (row - 1) * m % MOD * cnt % MOD + MOD) % MOD;
	}
	ll L = ((cl - 1) / 2) * ((cl - 1) / 2) + (cl - 1) / 2;
	ll R = (cr / 2) * (cr / 2) + cr / 2;
	ll cnt = len % 2 == 0 || cl & 1 ? len / 2 : len / 2 + 1;
	ll base = ((R - L) % MOD + cnt * m % MOD) % MOD;
	return (base % MOD + (row - 2) * m % MOD * cnt % MOD + MOD) % MOD;
}

ll f(ll r1, ll r2, ll cl, ll cr) {
	ll s1 = cal(r1, cl, cr);
	ll s2 = cal(r1 + 1, cl, cr);
	ll len = cr - cl + 1;
	ll odd_cnt = len % 2 == 0 || cl % 2 == 0 ? len / 2 : len / 2 + 1;
	ll even_cnt = len % 2 == 0 || cl & 1 ? len / 2 : len / 2 + 1;
	ll odd_delta = odd_cnt * 2 * m % MOD;
	ll even_delta = even_cnt * 2 * m % MOD;
	ll k = (r2 - r1 + 1) / 2;
	ll num = (k - 1) * k / 2 % MOD;
	// s1, s1 + odd_delta, s1 + 2 * odd_delta ... s1 + (k - 1) * odd_delta
	s1 = s1 * k % MOD + num * odd_delta % MOD;
	s2 = s2 * k % MOD + num * even_delta % MOD;
	if ((r2 - r1 + 1) & 1) return ((s1 + s2) % MOD + cal(r2, cl, cr)) % MOD;
	return (s1 + s2) % MOD;
}
