// Problem: C - Almost Sorted
// Contest: AtCoder - AtCoder Regular Contest 132
// URL: https://atcoder.jp/contests/arc132/tasks/arc132_c
...
int dfs(int p, int mask) {
	...
	if (p == N) return mask == 0 ? 1 : 0;
	int& res = dp[p][mask];
	res = 0;
	// [p-d,p-d+1,p-d+2,...p,p+1,p+2,p+3...p+d]
	// [0, 1, 2, 3, 4,...,d,d+1,d+2,d+3...2*d]
	for (int i = 0; i <= 2 * d; ++i) {
		if (mask >> i & 1) {
			int pos = p - d + i;
			if (abs(a[pos] - b[p]) <= d) {
				...
				if (!(nxt & 1)) {
					nxt >>= 1;
					if (p + 1 + d < N) nxt |= 1 << (2 * d);
					res = (res + dfs(p + 1, nxt)) % MOD;
				}
			}
		}
	}
