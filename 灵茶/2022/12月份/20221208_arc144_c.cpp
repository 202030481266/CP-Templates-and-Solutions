...
void solve() {
	...
	int x = n / m - 1;
	vi ans(n + 1), rk(m + 1);
	for (int i = 1; i <= m - k; ++i) rk[i] = i + k;
	for (int i = m - k + 1; i <= m; ++i) rk[i] = i - k;
	// [1, m], [m+1, 2m], ...[(x-1)*m+1, x*m]
	for (int i = 1; i <= x; ++i) {
		for (int j = 1; j <= m; ++j) ans[(i - 1) * m + j] = rk[j] + (i - 1) * m;
	}
	// [x*m+1, n] = [x*m+1, n-m] + [n-m+1, n] = [x*m+1, n-m] + [n-m+1, (x+1)*m] + [(x+1)*m+1, n]
	for (int i = x * m + 1; i <= n - m; ++i) ans[i] = rk[i - x * m] + x * m;
	vi arr{ 0 };
	for (int i = n - m + 1; i <= (x + 1) * m; ++i) arr.push_back(rk[i - x * m] + x * m);
	for (int i = (x + 1) * m + 1; i <= n; ++i) arr.push_back(i);
	ranges::sort(arr);
	for (int i = n - m + 1; i <= n; ++i) ans[i] = arr[rk[i - n + m]];
