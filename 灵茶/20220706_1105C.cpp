...
void solve() {
	...
	cnt[0] = r / 3 - (l - 1) / 3;
	cnt[1] = (r - 1) / 3 - (l + 1) / 3 + 1;
	cnt[2] = r < 2 ? 0 : (r - 2) / 3 - l / 3 + 1;
	dp[0] = cnt[0];
	dp[1] = cnt[1];
	dp[2] = cnt[2];
	for (int i = 1; i < n; ++i) {
		ll c0, c1, c2;
		c0 = (dp[2] * cnt[1] % MOD + dp[1] * cnt[2] % MOD + dp[0] * cnt[0] % MOD) % MOD;
		c1 = (dp[2] * cnt[2] % MOD + dp[1] * cnt[0] % MOD + dp[0] * cnt[1] % MOD) % MOD;
		c2 = (dp[2] * cnt[0] % MOD + dp[1] * cnt[1] % MOD + dp[0] * cnt[2] % MOD) % MOD;
		dp[0] = c0;
		dp[1] = c1;
		dp[2] = c2;
	}
