...
void solve() {
	...
	auto dfs = [&](auto&& self, int u, int fa) -> void {
		for (auto [v, w] : g[u]) {
			if (v != fa) {
				self(self, v, u);
				ll val = max(w + d[v], w + mx[v][0]);
				if (val > mx[u][0]) {
					mx[u][1] = mx[u][0];
					mx[u][0] = val;
				}
				else if (val > mx[u][1]) mx[u][1] = val;
			}
		}
	};
	dfs(dfs, 1, 0);
	auto f = [&](auto&& self, int u, int fa) -> void {
		for (auto [v, w] : g[u]) {
			if (v != fa) {
				ll son_val = w + max(d[v], mx[v][0]);
				if (mx[u][0] == son_val) dp[v] = w + max(dp[u], max(d[u], mx[u][1]));
				else dp[v] = w + max(dp[u], max(d[u], mx[u][0]));
				self(self, v, u);
			}
		}
	};
