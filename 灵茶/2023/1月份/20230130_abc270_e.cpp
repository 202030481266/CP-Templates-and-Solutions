...
void solve() {
	...
	while (p < n) {
		int i = p + 1;
		while (i < n && arr[ids[p]] == arr[ids[i]]) ++i;
		// [p, n)
		if (k > (n - p) * (arr[ids[p]] - last)) {
			k -= (n - p) * (arr[ids[p]] - last);
			tot += arr[ids[p]] - last;
			last = arr[ids[p]];
			for (int j = p; j < i; ++j) arr[ids[j]] = 0;
		}
		else {
			ll t = k / (n - p);
			for (int j = p; j < n; ++j) arr[ids[j]] -= t + tot;
			k %= n - p;
			for (int j = 0; j < n && k; ++j) {
				if (arr[j] > 0) {
					--arr[j];
