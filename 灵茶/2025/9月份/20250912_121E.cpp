...
int lazy[318], a[maxn];
int cnt[318][10003];
int bl[318], br[318], belong[maxn], mx[318];
int isluck[10005];
...
int main() {
	...
	for (int i = 0; i < 30; ++i) isluck[LUCKY_NUMBERS[i]] = 1;
		...
		if (op[0] == 'a') {
			...
			if (belong[l] == belong[r]) {
				for (int j = l; j <= r; ++j) {
					...
					a[j] += add;
					++cnt[belong[l]][a[j]];
					mx[belong[l]] = max(mx[belong[l]], a[j]);
				}
			}
			else {
				for (int j = l; j <= br[belong[l]]; ++j) {
					--cnt[belong[l]][a[j]];
					a[j] += add;
					++cnt[belong[l]][a[j]];
					mx[belong[l]] = max(mx[belong[l]], a[j]);
				}
				for (int j = bl[belong[r]]; j <= r; ++j) {
					--cnt[belong[r]][a[j]];
					a[j] += add;
					++cnt[belong[r]][a[j]];
					mx[belong[r]] = max(mx[belong[r]], a[j]);
				}
				for (int j = belong[l] + 1; j < belong[r]; ++j) lazy[j] += add;
			}
		}
		else {
			int ans = 0;
			if (belong[l] == belong[r]) {
				for (int j = l; j <= r; ++j) {
					if (isluck[a[j] + lazy[belong[l]]]) ++ans;
				}
			}
			else {
				for (int j = l; j <= br[belong[l]]; ++j) {
				...
				for (int j = bl[belong[r]]; j <= r; ++j) {
					if (isluck[a[j] + lazy[belong[r]]]) ++ans;
				}
				for (int j = belong[l] + 1; j < belong[r]; ++j) {
					for (int k = 0; k < 30; ++k) {
						if (LUCKY_NUMBERS[k] >= lazy[j]) {
							ans += cnt[j][LUCKY_NUMBERS[k] - lazy[j]];
						}
					}
				}
			}
