...
void solve() {
	...
	sort(ids + 1, ids + 1 + n, [&](int i, int j) { return arr[i] < arr[j]; });
	for (int i = 1; i <= n; ++i) sum[i] = sum[i - 1] + arr[ids[i]];
	for (int i = 1; i <= n; ++i) {
		int x = ids[i];
		ans[x] = (1ll * arr[x] * i - sum[i] + i) + (sum[n] - sum[i] - 1ll * arr[x] * (n - i) + n - i);
	}
