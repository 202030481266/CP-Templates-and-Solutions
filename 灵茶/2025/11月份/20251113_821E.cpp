...
static constexpr int maxmat = 16;
struct Mat {
	int n, m;
...
int n;
ll k, le[MAXN], ri[MAXN], he[MAXN];


void solve() {
	read(n); read(k);
	for (int i = 1; i <= n; ++i) {
		read(le[i]); read(ri[i]); read(he[i]);
	}
	Mat X(16, 1);
	X.at(0, 0) = 1;
	Mat A(16, 16), B(16, 16);
	for (int i = 1; i < n; ++i) {
		...
		if (ri[i] - le[i] > 1) {
			for (int j = 0; j <= he[i]; ++j) {
				A.at(j, j) = 1;
				if (j - 1 >= 0) A.at(j, j - 1) = 1;
				if (j + 1 <= he[i]) A.at(j, j + 1) = 1;
			}
			X = A.pow(ri[i] - le[i] - 1) * X;
		}
		for (int j = 0; j <= min(he[i], he[i + 1]); ++j) {
			B.at(j, j) = 1;
			if (j - 1 >= 0) B.at(j, j - 1) = 1;
			if (j + 1 <= he[i]) B.at(j, j + 1) = 1;
		}
		X = B * X;
	}
	if (k > le[n]) {
		...
		for (int i = 0; i <= he[n]; ++i) {
			A.at(i, i) = 1;
			if (i - 1 >= 0) A.at(i, i - 1) = 1;
			if (i + 1 <= he[n]) A.at(i, i + 1) = 1;
		}
		X = A.pow(k - le[n]) * X;
	}
	writeln(X.at(0, 0));
