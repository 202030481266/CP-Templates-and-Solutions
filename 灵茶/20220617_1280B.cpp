...
char grid[61][61];
int rmx, rmn, cmx, cmn, r, c;

bool check(int type, int l, int r, int x) {
	if (type) {
		for (int i = l; i <= r; ++i) {
			if (grid[x][i] != 'A') return false;
		}
	}
	else {
		for (int i = l; i <= r; ++i) {
			if (grid[i][x] != 'A') return false;
		}
	}
	return true;
}
...
void solve() {
	...
	for (int i = 1; i <= r; ++i) ans = min(ans, drawRow(i));
	for (int i = 1; i <= c; ++i) ans = min(ans, drawCol(i));
	if (ans <= 2) {
		writeln(ans);
		return;
	}
	if (grid[1][1] == 'A' || grid[1][c] == 'A' || grid[r][1] == 'A' || grid[r][c] == 'A') {
		writeln(2);
		return;
	}
	for (int i = 2; i < c; ++i) {
		if (grid[1][i] == 'A' || grid[r][i] == 'A') {
			if (i < cmn || i > cmx) ans = min(ans, 2);
			else ans = min(ans, 3);
		}
	}
	for (int i = 2; i < r; ++i) {
		if (grid[i][1] == 'A' || grid[i][c] == 'A') {
			if (i < rmn || i > rmx) ans = min(ans, 2);
			else ans = min(ans, 3);
		}
	}
	writeln(ans);
