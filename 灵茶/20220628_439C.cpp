...
void solve() {
	...
	int odd = a.size(), even = b.size(), c = max(0, p - even);
	odd -= 2 * c;
	if (odd < k - p) {
		writeln("NO");
		return;
	...
	if ((odd - k + p) % 2 == 0) {
		if (p > 0) {
			if (b.size() >= p) {
				...
				for (int i = k - p; i < a.size(); ++i) b.push_back(a[i]);
				for (int i = 0; i < p - 1; ++i) {
					write(1, ' ');
					writeln(b[i]);
				}
				write(b.size() - p + 1, ' ');
				for (int i = p - 1; i < b.size(); ++i) write(b[i], ' ');
			}
			else {
				int rm = a.size() - k + p;
				if (rm / 2 + b.size() >= p) {
					...
					int st = k - p + 2 * (c - 1);
					write(a.size() - st, ' ');
					for (int i = st; i < a.size(); ++i) write(a[i], ' ');
				}
				else writeln("NO");
			}
		}
			...
			write(a.size() - k + p + 1, ' ');
			for (int i = k - p - 1; i < a.size(); ++i) write(a[i], ' ');
		}
	}
	else writeln("NO");
}
