...
void solve() {
	...
	for (int i = 2; i <= n; ++i) {
		int x = arr[i - 1] / arr[i];
		if (x == 1) {
			// gcd(a[i-1], b[i]) = a[i] = a[i-1], b[i] = k * a[i]
			ans = m / arr[i] * ans % MOD;
		}
		else {
			// a[i-1] = x * a[i], b[i] = k * a[i], gcd(k, x) == 1
			for (int j = 2; j * j <= x; ++j) {
				if (x % j == 0) {
					while (x % j == 0) x /= j;
					factors[tot++] = j;
			...
			if (x > 1) factors[tot++] = x;
			int mx = m / arr[i]; // [1, mx] is valid
			ll res = 0;
			for (int j = 1; j < (1 << tot); ++j) {
				int b = popcount(1u * j), flag = b & 1 ? 1 : -1;
				ll mul = 1;
				for (int k = 0; k < tot; ++k) {
					if (j >> k & 1) mul *= factors[k];
				}
				res += mx / mul * flag;
			}
			ans = (mx - res) * ans % MOD;
