	...
	for (int i = 0; i < A.size(); ++i) {
		for (int j = 0; j < x[0].size(); ++j) {
			for (int k = 0; k < A[0].size(); ++k) {
				// A[0].size() == x.size()
				res[i][j] = (res[i][j] + 1ll * A[i][k] * x[k][j] % m) % m;
			}
		}
	}
	return res;
...
void solve() {
	...
	vii g(2, vi(1, 1));
	f[0][0] = 0;
	for (int i = 1; i < n - 1; ++i) f[0][0] = (f[0][0] + arr[i]) % p;
	if (x > 0) {
		A = quick_pow(A, x, p);
		f = mul(A, f, p); // 0
	}
	...
	f[0][0] = ((f[0][0] + arr[0]) % p + arr[n - 1]) % p; // sum of array after x operations
	ll mn, mx;
	if (x > 0) {
		mn = arr[0]; 
		mx = (g[0][0] * arr[n - 1] % p + g[1][0] * arr[n - 2] % p) % p; // the min/max element
	}
	else {
		mn = arr[0];
		mx = arr[n - 1];
	}
	f[0][0] = ((f[0][0] - mn - mx) % p + p) % p; // init for the y operations
	C[0][1] = (mx + mn) % p;
	if (y > 0) {
		C = quick_pow(C, y, p);
		f = mul(C, f, p);
	}
	ll ans = (mx + mn + f[0][0]) % p;
	writeln(ans);
}
