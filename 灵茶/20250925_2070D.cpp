...
int n, m, fa[maxn], dp[maxn], dep[maxn], cnt[maxn], arr[maxn];
...
void solve() {
	...
	fill_n(dep, n + 1, 0);
	fill_n(cnt, n + 1, 0);
	for (int i = 2; i <= n; ++i) read(fa[i]); // fa[1] = 0
	m = 0;
	for (int i = 1; i <= n; ++i) {
		dep[i] = dep[fa[i]] + 1;
		if (dep[i] > m) m = dep[i];
	...
	for (int i = 1; i <= n; ++i) arr[cnt[dep[i]]--] = i;
	fill_n(cnt, m + 1, 0); // [0, m]
	cnt[0] = 1;
	for (int i = 1; i <= n; ++i) {
		int x = arr[i];
		if (fa[x] <= 1) dp[x] = cnt[dep[x] - 1];
		else dp[x] = ((cnt[dep[x] - 1] - dp[fa[x]]) % mod + mod) % mod;
		cnt[dep[x]] = (cnt[dep[x]] + dp[x]) % mod;
	}
	int ans = 0;
