...
void solve() {
	...
	for (string& s : arr) {
		read(s);
		if (s.size() == 1) c.push_back(s[0]);
		if (s.size() == n - 1) b.push_back(s);
	}
	for (string& s : arr) {
		if (s.size() == n - 1 && (s[0] == c[0] || s[0] == c[1])) candi.emplace_back(s);
	}
	for (string& s : candi) {
		string full = s;
		if (s[0] == c[0]) full.push_back(c[1]);
		else full.push_back(c[0]);
		if (full.starts_with(b[0]) && full.ends_with(b[1]) || full.starts_with(b[1]) && full.ends_with(b[0])) {
			for (string& v : arr) {
				if (full.starts_with(v)) writeln("P");
				else writeln("S");
			}
			break;
		}
