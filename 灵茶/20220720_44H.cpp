...
ll dfs(int p, int pre, bool is_same) {
	if (p == n) return is_same ? 0 : 1;
	if (!is_same && dp[p][pre] != -1) return dp[p][pre];
	ll res = 0;
	if (p == 0) {
		for (int i = 0; i < 10; ++i) res += dfs(p + 1, i, is_same && i == s[p] - '0');
	}
	else {
		int cur = pre + s[p] - '0';
		if (cur & 1) {
			res += dfs(p + 1, cur / 2, is_same && cur / 2 == s[p] - '0');
			res += dfs(p + 1, cur / 2 + 1, is_same && cur / 2 + 1 == s[p] - '0');
		}
		else res += dfs(p + 1, cur / 2, is_same && cur / 2 == s[p] - '0');
	}
	if (!is_same) dp[p][pre] = res;
	return res;
}
