...
void solve() {
	...
	auto dfs = [&](auto&& self, int p, int lo_limit, int hi_limit) -> int {
		if (p == n) return 0;
		if (dp[p][lo_limit][hi_limit] > -1) return dp[p][lo_limit][hi_limit];
		int& res = dp[p][lo_limit][hi_limit], lo = 0, hi = 9;
		res = INF;
		if (lo_limit) lo = s[p] - '0';
		if (hi_limit) hi = t[p] - '0';
		for (int i = lo; i <= hi; ++i) {
			int cost = (i == s[p] - '0') + (i == t[p] - '0');
			res = min(res, cost + self(self, p + 1, lo_limit && i == lo, hi_limit && i == hi));
		}
		return res;
	};
	int ans = dfs(dfs, 0, 1, 1);
	writeln(ans);
}
