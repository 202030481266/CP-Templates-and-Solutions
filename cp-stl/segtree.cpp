...
struct S {
	...
	S() : a(INF), b(INF), c(INF), ab(INF), ac(INF), bc(INF), abc(INF) {}
};
...
S op(S l, S r) {
	...
	// ab + b
	// a + ab/b
	res.ab = min(min(l.ab + r.b, l.a + r.ab), l.a + r.b);
	res.ac = min(min(l.ac + r.c, l.a + r.ac), l.a + r.c);
	res.bc = min(min(l.bc + r.c, l.b + r.bc), l.b + r.c);
	// abc + c
	// ab + bc/c
	// a + abc/bc
	res.abc = min(min(l.abc + r.c, l.ab + r.bc, min(l.ab + r.c, l.a + r.abc)));
	res.abc = min(res.abc, l.a + r.bc);
}

// op(a, e()) == a && op(e(), a) == a
S e() {
	return { 0, 0, 0, INF, INF, INF, INF };
}
