...
- **开始写题**：[template.cpp](template.cpp) · [起手式用法](docs/usage/template.md)，或输入 `cpstart` 选择代码片段。
...
检查全部示例与使用文档，运行 `python cp-stl/tests/test_examples.py`（Python 3.10+）。它会核对每个头文件的说明、文档代码、预期输出与链接，并以 GNU C++17 编译运行全部 38 个示例，额外检查关闭 `LOCAL` 后调试输出为空。
