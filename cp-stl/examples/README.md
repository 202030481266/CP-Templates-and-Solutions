...
下面 35 个模板示例都内置数据：打开 `.cpp` 后按 `Ctrl+Shift+B` 即可运行，不需要输入。对应 `.ans` 文件记录标准输出；`util/debug.cpp` 还会向标准错误输出内容，对照 [debug.err](util/debug.err)。
...
在仓库根目录执行，将编译运行全部 38 个示例、核对输出，并检查所有模板都有使用文档且文档里的代码与示例一致。依赖 Python 3.10+，编译器从 `.vscode/settings.json` 读取，也可通过 `--compiler` 指定。
