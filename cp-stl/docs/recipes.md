...
以下是组合用法片段，假设使用 [template.cpp](../template.cpp) 的头文件和主函数，已包含对应算法头文件、声明题目变量，并把操作语句放进 `solve()`。数组从 0 开始，数组与字符串区间左闭右开。
...
根目录原有的 [ac-library](../../ac-library/README.md) 提供进阶组件。任务已加入其头文件路径，导出器也支持 `<atcoder/...>`。本库 35 个头文件本身不依赖 ACL；若重新克隆后 ACL 尚未初始化，可以先使用核心模板。

| 需求 | ACL 头文件 / 接口 | 本地文档 |
| --- | --- | --- |
| 任意懒标记、仿射变换 | `<atcoder/lazysegtree>` | [lazysegtree](../../ac-library/document_en/lazysegtree.md) |
| 最小费用最大流 | `<atcoder/mincostflow>` | [mincostflow](../../ac-library/document_en/mincostflow.md) |
| 后缀数组、LCP | `<atcoder/string>`，`suffix_array / lcp_array` | [string](../../ac-library/document_en/string.md) |
| 卷积 / NTT | `<atcoder/convolution>` | [convolution](../../ac-library/document_en/convolution.md) |
| CRT / floor_sum | `<atcoder/math>` | [math](../../ac-library/document_en/math.md) |

ACL 懒标记的 `composition(f,g)` 表示**先作用 g，再作用 f**；区间求和的节点要记录长度。最小费用流等组件还有各自的前置条件，使用前查看对应文档。
