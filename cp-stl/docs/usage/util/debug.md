...
在本地快速打印变量、容器、pair 和 tuple，输出走 stderr，正常答案仍走 stdout。仓库运行任务自动定义 LOCAL；不定义 LOCAL 时 debug 调用不产生输出，也不求值参数。
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File cp-stl/tools/cp.ps1 -Action Run -Source "cp-stl/examples/util/debug.cpp"
```
