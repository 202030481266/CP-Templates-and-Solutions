...
| `cross(a,b,c)` | 返回向量 ab 与 ac 的叉积，__int128；正/负/零分别表示左转/右转/共线。 |
...
- __int128 不能直接送给 cout；示例结果很小才安全地转换成 long long。
- 如果题目要求保留边界上的全部共线点，需要调整模板的弹栈条件及退化情况处理。

也可以在仓库根目录执行：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File cp-stl/tools/cp.ps1 -Action Run -Source "cp-stl/examples/geometry/convex_hull.cpp"
```
