...
struct BIT2D {
    ...
    void upd1(int x, int y1, int y2, long long mul, long long add) { // 懒标记，乘法和加法混合
...
int main() {
    ...
    while (q--) {
        ...
        if (ty == 1) {
            ...
            t.upd(
                x1, y1, x2, y2,
                val); // add val from top-left(x1, y1) to bottom-right (x2, y2);
        } else {
            cin >> x1 >> y1 >> x2 >> y2;
            cout << t.query(x1, y1, x2, y2)
                 << '\n'; // output sum from top-left(x1, y1) to bottom-right
                          // (x2, y2);
        }
    }
    return 0;
