...
namespace ntt
{
    ...
    public:
        ...
        static word_t init(word_t w)
        {
            return reduce(dword_t(w) * r2);
        }
        ...
        static Mod omega()
        {
            return Mod(prim_root).pow((mod - 1) >> level);
        }
...
public:
    ...
    static R add_mod(R a, R b)
    {
        return int(a += b - mod) < 0 ? a + mod : a;
    }
    static R sub_mod(R a, R b)
    {
        return int(a -= b) < 0 ? a + mod : a;
    }
    ...
    static R mul_mod(R a, R b)
    {
        return R64(a) * b % fast_mod;
    }
    ...
    struct fast_div
    {
        ...
        friend u64 operator / (u64 n, fast_div d)
        {
    ...
    struct fast_div
    {
        fast_div() {}
        fast_div(u32 n) : m(n) {}
        friend u32 operator % (u64 n, fast_div d)
        {
            return n % d.m;
        }
        u32 m;
