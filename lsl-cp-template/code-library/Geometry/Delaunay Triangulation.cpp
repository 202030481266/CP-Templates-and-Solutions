...
struct PT
{
    ...
    long long cross(PT p) const
    {
        return x * p.y - y * p.x;
    }
    long long cross(PT a, PT b) const
    {
...
struct dPT
{
    ...
    {
        return dPT(x + a.x, y + a.y);
    }
    dPT operator - (const dPT &a) const
    {
        return dPT(x - a.x, y - a.y);
    }
    dPT operator * (const double a) const
    {
        return dPT(x * a, y * a);
    }
    double norm2()
    ...
    double cross(dPT p) const
    {
        return x * p.y - y * p.x;
    }
    double cross(dPT a, dPT b) const
    {
        return (a - *this).cross(b - *this);
    }
};
inline double dot(dPT a, dPT b)
{
    return a.x * b.x + a.y * b.y;
}
inline double dist2(dPT a, dPT b)
{
    return dot(a - b, a - b);
}
inline double dist(dPT a, dPT b)
{
    return sqrt(dot(a - b, a - b));
}
inline double cross(dPT a, dPT b)
{
    return a.x * b.y - a.y * b.x;
}
dPT rotatecw90(dPT a)
{
    return dPT(a.y, -a.x);
}
