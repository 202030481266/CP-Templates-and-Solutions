...
int a[5010], b[5010];
    ...
    auto tree_max1 = OY::make_SegTree<2000000>(a, a + 5010, my_max);
    auto tree_max2 = OY::make_SegTree<2000000>(b, b + 5010, my_max);
#else
    struct {
...
public:
    int maxProfit(vector<int>& prices, vector<int>& profits) {
        ...
        tree_max1.s_use_count = 1;
        memset(a, -1, sizeof(a));
        memset(b, -1, sizeof(b));
        tree_max1.reset(a, a + 5010);
        tree_max2.reset(b, b + 5010);
    
        for (int i = 0; i < n; ++i) {
            int mx = tree_max2.query(0, prices[i] - 1);
            if (mx > 0)
                ans = max(ans, mx + profits[i]);
            int mx_val = tree_max1.query(0, prices[i] - 1);
            // update
            if (a[prices[i]] < profits[i]) {
                a[prices[i]] = profits[i];
                tree_max1.modify(prices[i], profits[i]);
            }
            if (mx_val > -1 && b[prices[i]] < mx_val + profits[i]) {
                b[prices[i]] = mx_val + profits[i];
                tree_max2.modify(prices[i], mx_val + profits[i]);
            }
        }
        return ans;
    }
