...
typedef unordered_map<string, bignum> E;
class Solution {
public:
    E dfs(string& s, int& p) {
        E res;
        for (; p < s.size(); ++p) {
            if (s[p] == ')') {
                bignum tim = 0;
                ++p;
                while (p < s.size() && isdigit(s[p])) tim = tim * 10 + s[p++] - '0';
                if (tim > 0) {
                    for (auto it = res.begin(); it != res.end(); ++it) {
                        it->second = it->second * tim;
                    }
                }
                // now p is return
                break;
                ...
                string atom{s[p]};
                ++p;
                while (p < s.size() && islower(s[p])) atom.push_back(s[p++]);
                // is there a number of the atom
                bignum tim = 0;
                while (p < s.size() && isdigit(s[p])) tim = tim * 10 + s[p++] - '0';
                if (tim > 0) res[atom] += tim;
                else res[atom] += 1; 
                --p;
            }
            else {
    ...
    string countOfAtoms(string formula) {
        // 使用栈来解表达式问题或者dfs
        // 结果感觉会产生一个非常大的数字，感觉使用python会比较nice
        // c++ 高精度模板
        int p = 0;
        E rs = dfs(formula, p);
        vector<string> keys;
        ...
        for (string& key : keys) {
            ...
            bignum tmp = rs[key];
            if (tmp == 1) continue;
            for (int i = tmp.len; i >= 1; --i) ans += to_string(tmp.data[i]);
        }
        return ans;
    }
};